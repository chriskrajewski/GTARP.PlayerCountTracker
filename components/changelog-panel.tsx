...
export function ChangelogPanel({ isOpen }: ChangelogPanelProps) {
  ...
  if (error) {
    return (
      <div className="bg-red-500/10 border border-red-500/40 p-4 rounded-md flex items-start gap-3">
        ...
        <div>
          <p className="text-red-300 font-medium">Unable to load changelog</p>
  ...
  return (
    <div className="space-y-4">
      {commits.length === 0 ? (
        <div className="flex items-center justify-center py-8">
          <p className="text-[#ADADB8]">No changelog entries available</p>
        </div>
      ) : (
        <div className="space-y-4">
          {commits.map((commit, index) => (
            <div key={commit.id}>
              <div className="flex flex-col space-y-1">
                ...
                <p className="text-sm text-[#EFEFF1] whitespace-pre-wrap break-words">{commit.message}</p>
              </div>
